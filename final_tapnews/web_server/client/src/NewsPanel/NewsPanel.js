	...
	loadMoreNews() {
		...
		if(!this.props || (!this.props.topic && !this.props.keyword && !this.props.list)){
			...
			if(this.props.topic){
				...
				let url = 'http://localhost:3000/news/userId/' + Auth.getEmail() + '/topic/' + this.props.topic + '/pageNum/' + this.state.pageNum;
				...
				let url = 'http://localhost:3000/news/userId/' + Auth.getEmail() + '/pageNum/' + this.state.pageNum + '/search/' + this.props.keyword;
				//console.log(encodeURI(url));
				...
				if(this.props.list=='read_later'){
					...
					let url = 'http://localhost:3000/news/userId/' + Auth.getEmail() + '/list/' + this.props.list;
					//console.log(encodeURI(url));
					let require = new Request(encodeURI(url),{
						method: 'GET',
						headers: {
